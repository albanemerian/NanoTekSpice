...
nts::Xor::Xor(std::string const &value) {
    _name = value;
    MAX_PIN = 3;
...
size_t nts::Xor::getMaxPin() const {
    return 3;
}

nts::Tristate nts::Xor::getPrevValue() const {
    return nts::Tristate::Undefined;
}

std::string nts::Xor::getPinType(std::size_t pin) {
    return _pins[pin]._type;
}

nts::Tristate nts::Xor::getPinState(std::size_t pin) {
    return _pins[pin]._value;
}

nts::Tristate nts::Xor::computes(std::size_t pin1, std::size_t pin2, std::size_t out) {
    return nts::Xor_logic(getPinState(pin1), getPinState(pin2), out);
}

nts::Tristate nts::Xor::getNextValue() const {
    return nts::Tristate::Undefined;
}
...
void nts::Xor::setPinState(std::size_t pin, nts::Tristate state) {
